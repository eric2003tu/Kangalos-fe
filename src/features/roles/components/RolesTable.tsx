...
export default function RolesTable() {
  ...
  const columns = useMemo<MRT_ColumnDef<Role & { organisationUnit?: { name?: string } }>[]>(
    () => [
      { accessorKey: "name", header: t("table.name") },
      { accessorKey: "description", header: t("table.description") },
      {
        accessorFn: (row) => row.organisationUnit?.name,
        id: "organisationUnit",
        header: t("table.organisationUnit"),
      },
      {
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline">{(
            rolesResponse as { meta?: { pagination?: { total?: number } } }
          )?.meta?.pagination?.total ?? 0} {t("total")}</Badge>
        </div>
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuCheckboxItem checked={columnVisibility.organisationUnit} onCheckedChange={(checked) => setColumnVisibility((prev) => ({ ...prev, organisationUnit: checked ?? false }))}>
                {t("table.organisationUnit")}
              </DropdownMenuCheckboxItem>
  ...
  return (
    <div className="space-y-4">
      ...
      <CreateRoleDialog isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} />

      {selectedRoleId && (
        <>
          <EditRoleDialog roleId={selectedRoleId} isOpen={showEditModal} onClose={() => { setShowEditModal(false); setSelectedRoleId(undefined); }} />
          <DeleteRoleDialog roleId={selectedRoleId} isOpen={showDeleteDialog} onClose={() => { setShowDeleteDialog(false); setSelectedRoleId(undefined); }} />
        </>
      )}
    </div>
  );
